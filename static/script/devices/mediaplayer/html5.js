    ...
    function(RuntimeContext, Device, MediaPlayer) {
        ...
        var Player = MediaPlayer.extend({
            ...
            */
            ...
            _exitBufferingSentinel: function (timeHasAdvanced) {
                var hasEnoughData = this._mediaElement.readyState === 4; // HAVE_ENOUGH_DATA
                if (timeHasAdvanced || this._mediaElement.paused || hasEnoughData) {
                    this._exitBuffering();
            ...
            _enterBufferingSentinel: function (timeHasAdvanced) {
                if (!timeHasAdvanced) {
                    this._toBuffering();
            ...
            _shouldBeSeekedToSentinel: function () {
                ...
                if (currentTime !== undefined && this._sentinelSeekTime !== undefined) {
                    var seekTime = this._readyToPlayFrom ? this._getClampedTime(this._sentinelSeekTime) : this._sentinelSeekTime;
                    if (Math.abs(currentTime - seekTime) > 15) { // Seek tolerance
                        this._mediaElement.currentTime = seekTime;
            ...
            _shouldBePausedSentinel: function (timeHasAdvanced) {
                if (timeHasAdvanced) {
                    this._mediaElement.pause();
            ...
            _shouldBePlayingSentinel: function (timeHasAdvanced) {
                if (!timeHasAdvanced && !this._isAtEndOfMedia()) {
                    this._mediaElement.play();
            ...
            _endOfMediaSentinel: function (timeHasAdvanced) {
                if (!timeHasAdvanced && this._isAtEndOfMedia()) {
                    this._toComplete();
